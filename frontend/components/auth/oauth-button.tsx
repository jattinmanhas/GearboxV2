...
export function OAuthButton({ provider, variant = 'outline', className = '' }: OAuthButtonProps) {
  ...
  return (
    <Button
      ...
      disabled={loading}
      className={`flex-1 aspect-square ${className}`}
      title={`Continue with ${OAuthProviderNames[provider]}`}
      aria-label={`Continue with ${OAuthProviderNames[provider]}`}
    >
      {loading ? (
...
// Group of OAuth buttons for login/register pages
