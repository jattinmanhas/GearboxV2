...
import { UploadedImage, getPublicIdFromUrl } from "@/lib/image-upload";
import {
  X,
  Plus,
  Link as LinkIcon,
...
export function FlexibleImageInput({
  ...
  const handleImagesUpload = (uploadedImages: UploadedImage[]) => {
    const newImages: ImageItem[] = uploadedImages.map((img) => ({
      id: img.id,
      ...
      isCloudinary: !!(
        img.publicId ||
        (img.url && getPublicIdFromUrl(img.url))
      ),
      width: img.width,
      height: img.height,
    ...
    if (multiple) {
      ...
      // If not multiple, just take the last one
      if (newImages.length > 0) {
        onImagesChange([newImages[newImages.length - 1]]);
      }
    }
  ...
  const handleImageUpload = (uploadedImage: UploadedImage) => {
    handleImagesUpload([uploadedImage]);
  };
  ...
  const handleRemoveImage = (imageId: string) => {
    ...
    // Note: We no longer delete from Cloudinary immediately.
    // The parent component (category-form) will handle deletion on form submit.
    // This prevents accidental data loss if the user clicks "Cancel" after removing an image.

    // Still call the parent handler for state management
                    ...
                    <img
                      ...
                      onError={(e) => {
                        ...
                        target.parentElement!.innerHTML =
                          '<div class="text-destructive text-sm">Invalid image URL</div>';
                      }}
                    />
                  </div>
                </div>
