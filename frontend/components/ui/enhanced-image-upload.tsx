...
import { uploadImage, deleteImage, validateImageFile, DEFAULT_IMAGE_CONFIG, type UploadedImage, type ImageUploadConfig } from '@/lib/image-upload'
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      {showPreview && previewImages.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Uploaded Images ({previewImages.length})</CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <div key={image.id} className="relative group">
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <div className="text-white text-center p-2">
                      <p className="text-xs">{image.width}x{image.height}</p>
                    ...
                    <div className="mt-2 space-y-1">
                      {Object.entries(image.thumbnails).map(([size, url]) => (
                        <div key={size} className="flex items-center gap-2 text-xs text-muted-foreground">
                          <img src={url} alt={`${size} thumbnail`} className="w-4 h-4 rounded object-cover" />
