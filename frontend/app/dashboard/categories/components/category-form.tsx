...
export function CategoryForm({
  ...
  // Track images that need to be deleted on submit (deferred deletion)
  ...
  useEffect(() => {
    if (category) {
      ...
      // Reset deletion tracking when editing a different category
      setImagesToDelete([]);
    } else {
  ...
  const handleCategoryImagesChange = (newImages: ImageItem[]) => {
    // Track which images were removed (for deferred deletion)
    const removedImages = categoryImages.filter(
      (oldImg) => !newImages.find((newImg) => newImg.id === oldImg.id)
    );

    // Add removed images with publicIds to the deletion queue
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const publicId of imagesToDelete) {
        try {
          ...
          if (!deleteResponse.ok) {
            ...
            console.log("Successfully deleted removed image:", publicId);
          }
        } catch (err) {
      ...
      // Handle image cleanup if editing and image has changed
      if (
        category &&
        category.image_url &&
        category.image_url !== formData.image_url
      ) {
        // Only delete the old image if it wasn't already in the deletion queue
        const publicId =
          category.image_public_id || getPublicIdFromUrl(category.image_url);
        ...
        if (publicId && !imagesToDelete.includes(publicId)) {
          try {
            ...
            if (!deleteResponse.ok) {
              console.error("Failed to delete old image:", publicId);
            } else {
          ...
          if (filename) {
            try {
              ...
              if (!deleteResponse.ok) {
                console.error("Failed to delete local image");
              }
            } catch (err) {
